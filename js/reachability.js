class Reachability {
  ...
  static isRegionVisible(rname) {
    if (hideEmptyNodes && hideOOL && !reach.regions.has(rname))
      return false
    if (
      hideEmptyNodes &&
    ...
    if (!Reachability.regionMatchesSearch(rname, searchQuery))
      return false
    if (noTransit && Reachability.isTransitRegion(rname)) return false
    return true
  }
